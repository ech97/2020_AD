...
def birdeye_view(img_roi, src, dst, size):
    ...
    img_bird = cv2.warpPerspective(img_roi, Perspect, size, flags=cv2.INTER_LINEAR)
...
def draw_lane(window, Line, roi_Area_correct):
    ...
    # 점 찍을 x, y 좌표 설정
    left_ptr_a = np.array([np.transpose(np.vstack([Line.plot_left_x - Line.window_margin/5, Line.plot_y]))])
    left_ptr_b = np.array([np.flipud(np.transpose(np.vstack([Line.plot_left_x + Line.window_margin/5, Line.plot_y])))])

    left_ptr = np.hstack((left_ptr_a, left_ptr_b))

    right_ptr_a = np.array([np.transpose(np.vstack([Line.plot_right_x - Line.window_margin/5, Line.plot_y]))])
    right_ptr_b = np.array([np.flipud(np.transpose(np.vstack([Line.plot_right_x + Line.window_margin/5, Line.plot_y])))])
    ...
    # 이제 라인 사이 색칠
    fill_a = np.array([np.transpose(np.vstack([Line.plot_left_x + Line.window_margin/5, Line.plot_y]))])
    fill_b = np.array([np.flipud(np.transpose(np.vstack([Line.plot_right_x - Line.window_margin/5, Line.plot_y])))])
...
def line_detect(img_bird, img_bird_result, roi_Area_correct, Line, height):
    ...
    for i in range(Line.window_num):
        ...
        # 윈도우 색칠
        cv2.rectangle(window, (win_lt_low, win_low), (win_lt_high, win_high), Line.window_color, 2)
        cv2.rectangle(window, (win_rt_low, win_low), (win_rt_high, win_high), Line.window_color, 2)
...
def post_line_detect(img_bird, img_bird_result, roi_Area_correct, Line, height):
    ...
    leftx_min = Line.left_fit[0] * nonzero_y ** 2 + Line.left_fit[1] * nonzero_y + Line.left_fit[2] - window_margin
    leftx_max = Line.left_fit[0] * nonzero_y ** 2 + Line.left_fit[1] * nonzero_y + Line.left_fit[2] + window_margin
    rightx_min = Line.right_fit[0] * nonzero_y ** 2 + Line.right_fit[1] * nonzero_y + Line.right_fit[2] - window_margin
    rightx_max = Line.right_fit[0] * nonzero_y ** 2 + Line.right_fit[1] * nonzero_y + Line.right_fit[2] + window_margin


    # 이전엔 window안에 있는 흰 픽셀 좌표의 인덱스를 뺐다면
    # 지금은 방금 계산한 x값 범위안에 있는 흰 픽셀의 인덱스만 뺌
    win_lt_ind = ((nonzero_x >= leftx_min) & (nonzero_x <= leftx_max)).nonzero()[0]
    win_rt_ind = ((nonzero_x >= rightx_min) & (nonzero_x <= rightx_max)).nonzero()[0]
